...
\subsubsection{Precision and recall} \todo{Are precision and recall equally important for the client?  Would it also make sense to report F-measure (possible with a weight favoring either P or R if they are not equally important)?}
...
almost never refer to other persons. \todo{Didn't you say above persons are are often linked to other persons?}
...
information, but better on questions.\todo{ `Expertise in Chemistry Teaching - ECTN database', `Hoe kan ik aan stemkastjes komen?'}. It also
performs worse on Persons. \todo{`Starfish'}
