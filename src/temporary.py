...
class DocumentLinker(object):
    ...
    def get_links(self, document, vtype='textvectorizer', dtype='euclidean',
        ...
        if l_deval:
            lvals = self.link_devaluation(self.data, document, self.links)
        if t_deval:
            tvals = self.tag_devaluation(self.data, document, self.links)

        self.links = sorted(self.links, key=lambda x: x[1])

        true_tvals = [x[1] for x in tvals if x[0] in document['links']]
        true_lvals = [x[1] for x in lvals if x[0] in document['links']]
        false_tvals = [x[1] for x in tvals if x[0] not in document['links']]
        false_lvals = [x[1] for x in lvals if x[0] not in document['links']]
        ...
        return [true_lvals, true_tvals, false_lvals, false_tvals]
    ...
    def link_devaluation(self, data, new_doc, deltas):
        ...
        return  [(doc, lp(nd_type,dtype(doc))) for doc, d in deltas]
...
def run(vectorizer, distancetype, thresh, l_deval, t_deval, k_link, directory):
    ...
    for new_doc, datawrapper in data.test_data():
        ...
        vals = linker.get_links(new_doc, vtype=vectorizer, dtype=distancetype, threshold=thresh, \
    ...
    trange = max([max(tvals), max(ftvals)])
    lrange = max([max(lvals), max(flvals)])

    plt.subplot(2,2,1)
    plt.xlim([0, trange])
    plt.ylim([0,1])
    plt.hist(tvals, 25, range=[0, trange], color='g', weights=np.zeros_like(tvals) + 1. / float(len(tvals)))

    plt.subplot(2,2,2)
    plt.xlim([0, lrange])
    plt.ylim([0,1])
    plt.hist(lvals, 25, range=[0, lrange], color='g',weights=np.zeros_like(lvals) + 1. / float(len(lvals)))
    
    plt.subplot(2,2,3)
    plt.xlim([0, trange])
    plt.ylim([0,1])
    plt.hist(ftvals, 25, range=[0, trange], color='r',weights=np.zeros_like(ftvals) + 1. / float(len(ftvals)))

    plt.subplot(2,2,4)
    plt.xlim([0, lrange])
    plt.ylim([0,1])
    plt.hist(flvals, 25, range=[0, lrange],color='r',weights=np.zeros_like(flvals) + 1. / float(len(flvals)))
    #plt.scatter(lvals, tvals, c='g')
    plt.savefig("probabilities.svg", transparent = True)
